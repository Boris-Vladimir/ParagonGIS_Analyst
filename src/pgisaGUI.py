...
class Window(object):
    ...
    def __init__(self):
        ...
        self.master.title('Paragon GIS Analyst - ver. 1.7')  # window name
        icons = os.getcwd() + os.sep + "icons" + os.sep  # path to icons
        icon = icons + "maps.ico"
        self.master.iconbitmap(icon)  # window icon
        self.master.resizable(width=FALSE, height=FALSE)
        self.master.geometry("520x100")  # with menu bar change 100 to 120
...
class BusinessLogic(object):
    ...
    def __prepare_excel_final_lines(self, icon_or_polygon, sheet_name):
        ...
        for line in range(num_rows + 1):
            ...
            if lower_line[0] == 'tipo de produto':
                ...
                try:  # altered in 08 mar 2016
                    date_index = self.paragon_titles_row.index('hora inicial')
                except:
                    date_index = self.paragon_titles_row.index('hora de início')
                self.__build_sheet_name(date_index)
                self.final_excel.write_line(fst_line)

            #  altered in 10 mar 2016 ------------------------------------------
            #if self.line[date_index][:10] != self.first_date:  # new sheet
            if self.__excel_to_datetime(date_index).date() != self.first_date: # new sheet
                #if self.first_date < self.line[date_index][:10]:
                if self.first_date < self.__excel_to_datetime(date_index).date():
                    #self.first_date = self.line[date_index][:10]
                    self.first_date = self.__excel_to_datetime(date_index).date()
            # -------------------------------------------------------------------
                self.__build_sheet_name(date_index)
                self.final_excel.write_line(fst_line)
    ...
    def __excel_final_end_line(self, icon_or_polygon, n_sheets, date_index,
        ...
        for line in range(n_sheets):
            ...
            elif self.__excel_to_datetime(date_index).date() != self.first_date:
                #if self.first_date < self.line[date_index][:10]:
                if self.first_date < self.__excel_to_datetime(date_index).date():
                    #self.first_date = self.line[date_index][:10]
                    self.first_date = self.__excel_to_datetime(date_index).date()
    ...
    def __build_sheet_name(self, date_index):        
        if self.line[0] == 'Tipo de produto' or self.line[0] == 'Tipo de Produto':
            self.line = self.xls.get_next_line()
            self.first_date = self.__excel_to_datetime(date_index).date() # altered in 10 mar 2016              
        else:
            try:
                self.line = self.xls.get_next_line()
            except:
                pass  # no more lines exist
        sheet_name = self.__get_week_days(self.first_date)
        self.final_excel.add_sheet(sheet_name)
        self.xls.reset_counter()  # reset line counter

    def __excel_to_datetime(self, date_index):
        # implemented in 10 mar 2016
        try:
            return datetime(self.line[date_index][0], self.line[date_index][1],
                        self.line[date_index][2], self.line[date_index][3],
                        self.line[date_index][4], self.line[date_index][5])
        # implemented in 31 mar 2017
        except:
            return datetime(int(self.line[date_index][6:10]),
                            int(self.line[date_index][3:5]),
                            int(self.line[date_index][0:2]),
                            int(self.line[date_index][11:13]),
                            int(self.line[date_index][14:16]),
                            int(self.line[date_index][17:19]))
    ...
    def __get_week_days(self, _date):
        ...
        ext_month = {1: 'JAN', 2: 'FEB', 3: 'MAR', 4: 'ABR',
        ...
        return week_day[_date.weekday()] + str(_date.day) + ext_month[_date.month]
    ...
    def __paragon_line(self):
        ...
        except:  # new paragon excel
            dir_i = self.paragon_titles_row.index('direção') 
            prod_i = self.paragon_titles_row.index('número de produto do alvo')
            gdh_1_i = self.paragon_titles_row.index('hora de início')
            gdh_2_i = self.paragon_titles_row.index('hora de fim')
            cgi_1_i = self.paragon_titles_row.index('célula inicial')
            cgi_2_i = self.paragon_titles_row.index('célula final')
        ...
        date_time_1 = self.__excel_to_datetime(gdh_1_i)
        date_time_2 = self.__excel_to_datetime(gdh_2_i)
        hour_1 = str(format(date_time_1.hour, '02d')) + ':' + str(format(date_time_1.minute, '02d')) + ':' + str(format(date_time_1.second, '02d'))
        date_1 = str(format(date_time_1.day, '02d')) + '/' + str(format(date_time_1.month, '02d')) + '/' + str(date_time_1.year)
        hour_2 = str(format(date_time_2.hour, '02d')) + ':' + str(format(date_time_2.minute, '02d')) + ':' + str(format(date_time_2.second, '02d'))
        date_2 = str(format(date_time_2.day, '02d')) + '/' + str(format(date_time_2.month, '02d')) + '/' + str(date_time_2.year)

        self.paragon_line_1.append(hour_1)
        self.paragon_line_2.append(hour_2)
        ...
        self.paragon_line_1.append(date_1)
        self.paragon_line_2.append(date_2)
...
class CgiTimeFilter(object):
    ...
    def __calc_sec_diference(self, time_1, time_2):
        ...
        if 'd' in time_1:
            datetime_1 = datetime.strptime(time_1[:-1], fmt)
        else:
            datetime_1 = datetime.strptime(time_1, fmt)
        if 'd' in time_2:
            datetime_2 = datetime.strptime(time_2[:-1], fmt)
        else:
